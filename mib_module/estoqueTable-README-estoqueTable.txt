...
------------------------------------------------------------------------
  ...
  the estoqueTable table.  The agent will start by looking for the right
  ...
  file defaults/table-estoqueTable.m2d, and then re-running mib2c.
 ...
 Index(es) for the estoqueTable table
  ------------------------------------------------------------
  The index(es) for the estoqueTable table are:
...
************************************************************************
estoqueTable File Overview
------------------------------------------------------------------------
  Several files have been generated to implement the estoqueTable
  table. We'll go through these files, one by one, explaining each and
  letting you know which you need to edit.


File: estoqueTable_data_access.[c|h]
------------------------------------------------------------------------
  The estoqueTable_data_access file contains the interface to your data in
  ...
  FUNC : estoqueTable_indexes_set
  WHERE: estoqueTable_data_access.c
...
------------------------------------------------------------------------
  ...
  Set $m2c_table_dependencies = 0 in defaults/table-estoqueTable.m2d
...
NOTE: Depending on your configuration, some of the functions used in the
      ...
      were generated for the estoqueTable table.

To watch the flow of the estoqueTable table, use the
following debug tokens:

        snmp_agent
        helper:table:req
        estoqueTable
        verbose:estoqueTable
        internal:estoqueTable

e.g.
        snmpd -f -Le -DestoqueTable,verbose:estoqueTable,internal:estoqueTable
...
/*---------------------------------------------------------------------
 * RestCtrl-MIB::estoqueEntry.estoqueIndex
 * estoqueIndex is subid 1 of estoqueEntry.
 * Its status is Mandatory, and its access level is ReadWrite.
 * OID: .1.3.6.1.4.1.12619.9.1.1
 * Description:
