...
------------------------------------------------------------------------
  ...
  the mesasTable table.  The agent will start by looking for the right
  ...
  file defaults/table-mesasTable.m2d, and then re-running mib2c.
    ...
    will be generated:
      ...
      mesasTable_allocate_data
      mesasTable_release_data
 ...
 Index(es) for the mesasTable table
  ------------------------------------------------------------
  The index(es) for the mesasTable table are:
...
************************************************************************
mesasTable File Overview
------------------------------------------------------------------------
  Several files have been generated to implement the mesasTable
  table. We'll go through these files, one by one, explaining each and
  letting you know which you need to edit.


File: mesasTable_data_access.[c|h]
------------------------------------------------------------------------
  The mesasTable_data_access file contains the interface to your data in
  ...
  FUNC : mesasTable_indexes_set
  WHERE: mesasTable_data_access.c
...
File: mesasTable_data_get.c
------------------------------------------------------------------------
  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : capacidadeMesa_get

  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : numClientes_get
...
NOTE: Depending on your configuration, some of the functions used in the
      ...
      were generated for the mesasTable table.

To watch the flow of the mesasTable table, use the
...
e.g.
        snmpd -f -Le -DmesasTable,verbose:mesasTable,internal:mesasTable
...
/*---------------------------------------------------------------------
 * RestCtrl-MIB::mesasEntry.mesasIndex
 * mesasIndex is subid 1 of mesasEntry.
 * Its status is Mandatory, and its access level is ReadOnly.
 * OID: .1.3.6.1.4.1.12619.3.1.1
 * Description:
