...
------------------------------------------------------------------------
  ...
  the dailyOrdersTable table.  The agent will start by looking for the right
  ...
  file defaults/table-dailyOrdersTable.m2d, and then re-running mib2c.
    ...
    in the dailyOrdersTable.h header.
    ...
    will be generated:
      ...
      dailyOrdersTable_allocate_data
      dailyOrdersTable_release_data
 ...
 Index(es) for the dailyOrdersTable table
  ------------------------------------------------------------
  The index(es) for the dailyOrdersTable table are:
...
************************************************************************
dailyOrdersTable File Overview
------------------------------------------------------------------------
  Several files have been generated to implement the dailyOrdersTable
  table. We'll go through these files, one by one, explaining each and
  letting you know which you need to edit.


File: dailyOrdersTable_data_access.[c|h]
------------------------------------------------------------------------
  The dailyOrdersTable_data_access file contains the interface to your data in
  ...
  FUNC : dailyOrdersTable_indexes_set
  WHERE: dailyOrdersTable_data_access.c
...
File: dailyOrdersTable_data_get.c
------------------------------------------------------------------------
  Get data for column
  -------------------
...
File: dailyOrdersTable_data_set.c
------------------------------------------------------------------------
...
NOTE: Depending on your configuration, some of the functions used in the
      ...
      dailyOrdersTable table.

      Conversely, the examples below may not include some functions that
      were generated for the dailyOrdersTable table.

To watch the flow of the dailyOrdersTable table, use the
following debug tokens:

        snmp_agent
        helper:table:req
        dailyOrdersTable
        verbose:dailyOrdersTable
        internal:dailyOrdersTable

e.g.
        snmpd -f -Le -DdailyOrdersTable,verbose:dailyOrdersTable,internal:dailyOrdersTable
...
/*---------------------------------------------------------------------
 * RestCtrl-MIB::dailyOrdersEntry.dailyOrdersIndex
 * dailyOrdersIndex is subid 1 of dailyOrdersEntry.
 * Its status is Mandatory, and its access level is ReadOnly.
 * OID: .1.3.6.1.4.1.12619.8.1.1
 * Description:
