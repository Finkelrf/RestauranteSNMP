...
------------------------------------------------------------------------
  ...
  the menuTable table.  The agent will start by looking for the right
  ...
  file defaults/table-menuTable.m2d, and then re-running mib2c.
    ...
    will be generated:
      ...
      menuTable_allocate_data
      menuTable_release_data
 ...
 Index(es) for the menuTable table
  ------------------------------------------------------------
  The index(es) for the menuTable table are:
...
************************************************************************
menuTable File Overview
------------------------------------------------------------------------
  Several files have been generated to implement the menuTable
  table. We'll go through these files, one by one, explaining each and
  letting you know which you need to edit.


File: menuTable_data_access.[c|h]
------------------------------------------------------------------------
  The menuTable_data_access file contains the interface to your data in
  ...
  FUNC : menuTable_indexes_set
  WHERE: menuTable_data_access.c
...
File: menuTable_data_get.c
------------------------------------------------------------------------
  Get data for column
  -------------------
  TODO : retrieve column data from raw data
  FUNC : description_get



File: menuTable_data_set.c
------------------------------------------------------------------------
...
NOTE: Depending on your configuration, some of the functions used in the
      ...
      were generated for the menuTable table.

To watch the flow of the menuTable table, use the
...
e.g.
        snmpd -f -Le -DmenuTable,verbose:menuTable,internal:menuTable
...
/*---------------------------------------------------------------------
 * RestCtrl-MIB::menuEntry.menuIndex
 * menuIndex is subid 1 of menuEntry.
 * Its status is Mandatory, and its access level is ReadOnly.
 * OID: .1.3.6.1.4.1.12619.5.1.1
 * Description:
